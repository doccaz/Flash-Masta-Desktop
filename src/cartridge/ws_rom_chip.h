/*! \file
 *  \brief File containing the declaration of the \ref ws_rom_chip class.
 *  
 *  File containing the header information and declaration of the
 *  \ref ws_rom_chip class.
 *  
 ...
 *  device information.
 *  
 *  This class is inteded to be used as a communication utility and stores very
 *  little information about the current state of the device that it represents.
 *  Its functionality is also limited by how the hardware device responds to
 *  certain commands.
 *  
...
public:
  ...
  /*!
   *  \brief Type used for indicating whether a sector on the device is write
   *         protected.
   */
   ...
   *  
   *  Enumeration representing the current assume dstate of the device.
   *  
   *  \see current_mode()
   */
  enum chip_mode
...
public:
   ...
   *  parameters.
   *  
   *  \param[in,out] linkmasta_device Reference to the linkmasta device this
   *                 chip is to use for communication with the hardware. Must
   *                 be a pointer to valid object in memory.
   *  
   *  \see linkmasta_device
   */
                          ws_rom_chip(linkmasta_device* linkmasta_device);
  
  /*! \brief The destructor for the class.
   *  
   *  Frees dynamically allocated memory and closes any open connections.
   *  
   *  To prevent the device from being in an inconsistent state, be sure to
   *  call \ref reset() before allowing this object to be destroyed.
   *  
   *  \see rset()
   */
                          ~ws_rom_chip();
  ...
  /*! \brief Determines whether or not the chip supports \ref chip_mode::BYPASS
   *         mode.
   *  
   *  This class was designed to be compatible with only one specific model of
   *  flash storage device. Because of this and because it is known that the
   *  device supports bypass mode, this function does nothing more than return
   *  **true**. This function is largely intended to be a convenience function.
   *  
